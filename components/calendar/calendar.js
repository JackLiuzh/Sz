...
Component({
  ...
  properties: {
    ...
    selected: {
      ...
      observer(newVal, oldVal) {
        //缓存不存在 则取当前时间
        var currenttime = wx.getStorageSync('rili-currenttime')
        if (!currenttime) {
          currenttime = new Date()
        }
        this.getWeek(currenttime)
      }
    },
    isOpen: {
      type: Boolean,
  ...
  methods: {
    ...
    selectDay(e) {
      ...
      //缓存存 选择的日期
      wx.setStorageSync('rili-currenttime', canlender.year + "-" + month + "-" + date)
      //console.log(canlender.year + "-" + month + "-" + date+"-----------");
    ...
    dataBefor(e) {
      ...
      //  console.log(d)
        if (d.data.status == 0) {
          ...
          this.setData({ todaytask: d.data.data.todaytask })
          var selectedArray = this.data.selected
          rili.forEach(function (item) {
            if (new Date(item.date) < new Date(xitongtime)) {
              var obj = {}
              obj.date = item.date
          ...
          this.setData({ selected: selectedArray })
        }
      })
