...
Page({
   ...
   */
  data: {
    isshowtext: false, // false文本text显示
    currentTab: 0,
    clientHeight: 500,
    list: [
      {
      ...
      if (d.data.status == 0) {
        if (d.data.data.data.length) {
          that.setData({
            ...
            metailes: d.data.data.material_data,
          ...
          for (let i = 0; i < d.data.data.data.length; i++) {
            ...
            if ((d.data.data.data[i].user_answer) && (d.data.data.data[i].user_answer).length > 0) {
              ...
              var question = d.data.data.data[i]
              // console.log(xuanxiang, pindex, answer)
              that.isshowzuotido(xuanxiang, pindex, answer)
            }
          }
          that.setData({
            questions: d.data.data.data,
        ...
        if (d.data.data.material_data.length) {
          d.data.data.material_data.forEach(function(item,index){
            WxParse.wxParse('mate' + index, 'html', d.data.data.material_data[index].content,that)
            if (index === d.data.data.material_data.length - 1){
              WxParse.wxParseTemArray("mateTemArray", 'mate', d.data.data.material_data.length, that)
            }
          })
        }
  ...
  onPageScroll:function() {
    var dd = !this.data.gundonghide
    this.setData({ gundonghide: dd })
  },
  // 初始化屏幕高度
  initscreen: function () {
    var that = this
  ...
  choosed: function (e) {
    ...
    that.setData({ [a]: a_an, [b]: b_an, [c]: c_an, [d]: d_an, [isdo]: 1, [iswro]: iswrong, [du]: duration, [ave_du]: newave_du, [ro]: newro })
    ...
    that.setData({ [datiiswrong]: iswrong })
  ...
  //gometa
  gometa: function(e) {
      var swi = ! this.data.showmetail
      console.log(swi)
      this.setData({ showmetail: swi})
      
  },

  //is是否已经做过题
  isshowzuotido: function (xuanxiang, pindex, answer) {
    var that = this
    ...
    //console.log(a_an, b_an, c_an, d_an)
    that.setData({ [a]: a_an, [b]: b_an, [c]: c_an, [d]: d_an, [isdo]: 1, [iswro]: iswrong, [isfocus]: false })
  }
